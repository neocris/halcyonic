...
var MastodonAPI = function(config) {
  ...
  //var apiBase = "https://mstdn.io/api/v1/";
  return {
        ...
        for (var i in queryData) {
          if (queryData.hasOwnProperty(i)) {
            if (typeof queryData[i] === "string") {
              ...
              queryStringAppend +=
                queryData[i].name + "=" + queryData[i].data + "&";
            }
          }
        }
      }
        ...
        if (queryData.hasOwnProperty(i)) {
          if (typeof queryData[i] === "string") {
            ...
            for (var j in queryData[i].data) {
              queryStringAppend +=
                queryData[i].name + "[]=" + queryData[i].data[j] + "&";
            }
          }
        }
      }
      ...
      $.ajax({
        ...
        success: function(data, textStatus) {
          if (endpoint == "statuses" && data.visibility != "direct") {
            ...
            $(".js_current_following_count").html(
              ++localStorage.current_following_count
            );
    ...
    stream: function(streamType, onData) {
      var es = new WebSocket(
        "wss://" +
          apiBase.substr(8) +
          "streaming?access_token=" +
          config.api_user_token +
          "&stream=" +
          streamType
      );
