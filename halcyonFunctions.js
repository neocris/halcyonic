...
function getLinkFromXHRHeader(xhrheaderstring) {
  const re = xhrheaderstring.match(
    /link: <.+api\/v1\/(.+?)>; rel="(.+?)", <.+api\/v1\/(.+?)>; rel="(.+?)"/i
  );
  let di = new Object();
  if (re) {
    di[re[2]] = re[1];
...
function replaceInternalLink() {
  ...
  $(".toot_article a,.profile_bio a,.follows_profile_bio a").each(function(i) {
    const pltags = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/tag\/([a-zA-Z\d_%]+)\/?$/);
    if (pltags)
      $(this)
        .attr("target", "_self")
        .attr("href", "/search?q=" + pltags[1]);
    const mstags = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/tags\/([a-zA-Z\d_%]+)\/?$/);
    if (mstags)
      $(this)
        .attr("target", "_self")
        .attr("href", "/search?q=" + mstags[1]);
    const plusers = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/users\/([a-zA-Z\d_]+)(\/statuses\/\d+)\/?$/);
    if (plusers)
      $(this)
        .attr("target", "_self")
        .attr(
    ...
    const msstatus = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/@([a-zA-Z\d_]+)(\/\d+)\/?$/);
    if (msstatus)
      $(this)
        .attr("target", "_self")
        .attr("href", "javascript:openStatus('" + msstatus[0] + "');void(0)");
    const msstatus2 = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/users\/([a-zA-Z\d_]+)\/?$/);
    if (msstatus2)
      $(this)
        .attr("target", "_self")
        .attr("href", "javascript:openStatus('" + msstatus2[0] + "');void(0)");
    const gsstatus = $(this)
      .attr("href")
      .match(/https?:\/\/.+..+\/notice\/(\d+)\/?$/);
    if (gsstatus)
      $(this)
        .attr("target", "_self")
        .attr("href", "javascript:openStatus('" + gsstatus[0] + "');void(0)");
    const plstatus = $(this)
      .attr("href")
      .match(
        /https?:\/\/.+..+\/objects\/([\da-z]{8}-[\da-z]{4}-[\da-z]{4}-[\da-z]{4}-[\da-z]{12})\/?$/
      );
    if (plstatus)
      $(this)
        .attr("target", "_self")
        .attr("href", "javascript:openStatus('" + plstatus[0] + "');void(0)");
  });
  $(".toot_article a").each(function(i) {
    const ytcom = $(this)
      .attr("href")
      .match(/https?:\/\/(www\.)?youtube\.com\/watch\?v=([a-zA-Z\d_-]+)/);
    const htcom = $(this)
      .attr("href")
      .match(/https?:\/\/(www\.)?hooktube\.com\/watch\?v=([a-zA-Z\d_-]+)/);
    const ivcom = $(this)
      .attr("href")
      .match(/https?:\/\/(www\.)?invidio\.us\/watch\?v=([a-zA-Z\d_-]+)/);
    const ytbe = $(this)
      .attr("href")
      .match(/https?:\/\/(www\.)?youtu\.be\/([a-zA-Z\d_-]+)/);
    const htbe = $(this)
      .attr("href")
      .match(/https?:\/\/(www\.)?hooktube\.com\/([a-zA-Z\d_-]+)/);
    const vimeo = $(this)
      .attr("href")
    ...
    const peertube = $(this)
      .attr("href")
      .match(
        /https?:\/\/.+..+\/videos\/watch\/([\da-z]{8}-[\da-z]{4}-[\da-z]{4}-[\da-z]{4}-[\da-z]{12})\/?$/
      );
    if (ytcom)
      embedMedia("youtube", $(this).closest(".toot_article"), ytcom[2]);
    else if (htcom)
      embedMedia("youtube", $(this).closest(".toot_article"), htcom[2]);
    else if (ivcom)
      embedMedia("youtube", $(this).closest(".toot_article"), ivcom[2]);
    else if (ytbe)
      embedMedia("youtube", $(this).closest(".toot_article"), ytbe[2]);
    else if (htbe)
      embedMedia("youtube", $(this).closest(".toot_article"), htbe[2]);
    else if (vimeo)
      embedMedia("vimeo", $(this).closest(".toot_article"), vimeo[2]);
    else if (peertube)
      embedMedia(
        "peertube",
        $(this).closest(".toot_article"),
        peertube[0].replace("/watch/", "/embed/")
      );
        ...
        if (
          ...
          api.get(
            ...
            function(data) {
              cards[this_id] = data;
              if (
                $(".toot_entry[sid=" + this_id + "]")
                  .children()
                  .children("section")
                  .children("article")
                  .children(".link_preview").length == 0 &&
                data.url
              ) {
                $(".toot_entry[sid=" + this_id + "]")
                  .children()
...
function getRelativeDatetime(
  ...
  if (elapsedTime < 60) {
    ...
    const datetime =
      dot +
      calendar[posted_time_original.getMonth()] +
      " " +
      posted_time_original.getDate();
    return datetime;
  }
}
...
function resetApp() {
  ...
  api.get("accounts/verify_credentials", function(AccountObj) {
    ...
    localStorage.setItem(
      "current_url",
      getRelativeURL(AccountObj["url"], AccountObj["id"])
    );
    ...
    localStorage.setItem(
      "current_statuses_count",
      AccountObj["statuses_count"]
    );
    localStorage.setItem(
      "current_following_count",
      AccountObj["following_count"]
    );
    localStorage.setItem(
      "current_followers_count",
      AccountObj["followers_count"]
    );
    localStorage.setItem(
      "current_statuses_count_link",
      getRelativeURL(AccountObj["url"], AccountObj["id"])
    );
    localStorage.setItem(
      "current_following_count_link",
      getRelativeURL(AccountObj["url"], AccountObj["id"], "/following")
    );
    localStorage.setItem(
      "current_followers_count_link",
      getRelativeURL(AccountObj["url"], AccountObj["id"], "/followers")
    );
    localStorage.setItem(
      "current_favourites_link",
      getRelativeURL(AccountObj["url"], AccountObj["id"], "/favourites")
    );
    ...
    current_statuses_count_link = localStorage.getItem(
      "current_statuses_count_link"
    );
    current_following_count_link = localStorage.getItem(
      "current_following_count_link"
    );
    current_followers_count_link = localStorage.getItem(
      "current_followers_count_link"
    );
    ...
    current_search_history = JSON.parse(
      localStorage.getItem("current_search_history")
    );
    setCurrentProfile();
  });
...
function refreshApp() {
  ...
  current_instance_charlimit = localStorage.getItem(
    "current_instance_charlimit"
  );
  current_instance_charlimit = 1000;
...
function pushNotification(title, message) {
  if (
    window.Notification &&
    localStorage.setting_desktop_notifications == "true"
  ) {
...
function setWhoToFollow(sanimate) {
  if (sanimate == true) {
    $(".follow_opt_in").slideUp(function() {
      $(".follow_loading").slideDown();
    });
  } else {
    $(".follow_opt_in").hide();
    $(".follow_loading").show();
      ...
      .done(function(data) {
        ...
        if (data.status == 200) {
          ...
          for (i = 0; i < data.ids.length; i++) {
            if (
              current_following_accts.indexOf(data.ids[i].to_id) == -1 &&
              current_blocked_accts.indexOf(data.ids[i].to_id) == -1 &&
              current_muted_accts.indexOf(data.ids[i].to_id) == -1
            ) {
...
function addFollowProfile(id, account) {
  api.get(
    "search",
    [{ name: "q", data: "@" + account }, { name: "resolve", data: "true" }],
    function(search) {
      search.accounts[0].display_name = htmlEscape(
        search.accounts[0].display_name
      );
      for (i = 0; i < search.accounts[0].emojis.length; i++) {
        search.accounts[0].display_name = search.accounts[0].display_name.replace(
          new RegExp(":" + search.accounts[0].emojis[i].shortcode + ":", "g"),
          "<img src='" + search.accounts[0].emojis[i].url + "' class='emoji'>"
        );
      }
      ...
      if (search.accounts[0].acct.indexOf("@") == -1)
        wtf_account_link =
          "/@" +
          search.accounts[0].acct +
          "@" +
          current_instance +
          "?mid=" +
          search.accounts[0].id;
      else
        wtf_account_link =
          "/@" + search.accounts[0].acct + "?mid=" + search.accounts[0].id;
      $(".what_to_follow_" + id + " > .icon_box img").attr(
        "src",
        search.accounts[0].avatar
      );
      $(".what_to_follow_" + id + " .label_box > a").attr(
        "href",
        wtf_account_link
      );
      $(".what_to_follow_" + id + " .label_box > a > h3 .dn")
        .addClass("emoji_poss")
        .html(search.accounts[0].display_name);
      $(".what_to_follow_" + id + " .label_box > a > h3 .un").text(
        "@" + search.accounts[0].username
      );
      $(".what_to_follow_" + id + " .label_box > .follow_button").attr(
        "mid",
        search.accounts[0].id
      );
      $(".what_to_follow_" + id + " .label_box > .follow_button").attr(
        "data",
        search.accounts[0].url
      );
      follow_loaded++;
    }
  );
...
function embedMedia(source, element, watchid) {
  if (element.children(".media_views").length == 0) {
    let media_views = `<div class='media_views' sid="${element
      .parent()
      .parent()
      .parent()
      .attr("sid")}" media_length='1' style="border:0;border-radius:0">`;
    if (
      source == "youtube" &&
      server_setting_youplay == true &&
      localStorage.setting_play_youplay == "true"
    ) {
      media_views += `
...
<iframe src="/media/youplay.php?id=${watchid}" frameborder="0" allowfullscreen></iframe>
      ...
      source == "youtube" &&
      localStorage.setting_play_invidious == "true"
    ) {
      media_views += `
<div class="media_attachment" otype="video/gifv" mediacount="0">
<iframe src="https://www.invidio.us/embed/${watchid}" frameborder="0" allowfullscreen></iframe>
      ...
      source == "vimeo" &&
      server_setting_vimeo == true &&
      localStorage.setting_play_vimeo == "true"
    ) {
      media_views += `
<div class="media_attachment" otype="video/gifv" mediacount="0">
<iframe src="/media/vimeo.php?id=${watchid}" frameborder="0" allowfullscreen></iframe>
      ...
      source == "peertube" &&
      localStorage.setting_play_peertube == "true"
    ) {
      media_views += `
<div class="media_attachment" otype="video/gifv" mediacount="0">
...
function submitStatusArray(params, callback) {
  ...
  api.post("statuses", params, function(data) {
    statuses = statuses.filter(function(index) {
      return index != 0;
    });
