function mediaattachments_template(status) {
  ...
  if (
    ...
    localStorage.setting_play_video != "false"
  )
    border = ' style="border:0;border-radius:0"';
  if (
    localStorage.setting_full_height == "true" &&
    status.media_attachments.length == 1 &&
    (status.media_attachments[0].type == "image" ||
      (status.media_attachments[0].type === "video" &&
        localStorage.setting_play_video == "false") ||
      (status.media_attachments[0].type === "gifv" &&
        localStorage.setting_play_gif == "false"))
  ) {
  ...
  if (status.media_attachments[0].url === "/files/original/missing.png") {
    ...
    media_views = `<div class='media_views${mvfullheight}' sid="${
      status.id
    }" media_length='${dsplength}'${border}>`;
  } else {
...
<span class="text2">${__("Click to view")}</span>
  ...
  if (
    ...
    media_views += `
<div class="media_attachment" otype="image" sid="${status.id}" oid="${
      status.media_attachments[0].id
    }" url="${status.media_attachments[0].preview_url}" mediacount="0">
...
<iframe src="/media/video.php?url=${encodeURIComponent(
...
<img src="${status.media_attachments[0].preview_url}">
    ...
    status.media_attachments[0].type === "audio" ||
    (status.media_attachments[0].type === "unknown" &&
      status.media_attachments[0].url.substring(
        status.media_attachments[0].url.length - 4
      ) == ".mp3")
  ) {
    ...
    if (status.media_attachments.length <= 2) {
      for (let i in status.media_attachments) {
        if (status.media_attachments[i].remote_url != null) {
          status.media_attachments[i].url =
            status.media_attachments[i].remote_url;
        }
        media_views += `
<div class="media_attachment" otype="image" sid="${status.id}" oid="${
          status.media_attachments[i].id
        }" url="${status.media_attachments[i].url}" mediacount="${i}">
...
function poll_template(poll) {
  ...
  else if (expires_at.getUTCDate() > 2)
    expires_string = expires_at.getUTCDate() - 1 + " " + __("days");
  else if (expires_at.getUTCHours() == 1) expires_string = "1 " + __("hour");
  else if (expires_at.getUTCHours() > 1)
    expires_string = expires_at.getUTCHours() + " " + __("hours");
  else if (expires_at.getUTCMinutes() == 1)
    expires_string = "1 " + __("minute");
  else if (expires_at.getUTCMinutes() > 1)
    expires_string = expires_at.getUTCMinutes() + " " + __("minutes");
  else if (expires_at.getUTCSeconds() == 1)
    expires_string = "1 " + __("second");
  ...
  if (poll.voted || poll.expired) {
    ...
    optionsort.sort(function(a, b) {
      return a.votes_count - b.votes_count;
    });
    optionsort.reverse();
    if (optionsort[0].votes_count != optionsort[1].votes_count)
      poll.options[poll.options.indexOf(optionsort[0])].winner = true;
    for (var i = 0; i < poll.options.length; i++) {
      var winner = "";
      if (poll.options[i].winner) winner = " poll_winner";
      poll_html += `<div class="poll_result_option"><span class="poll_bar${winner}" style="width:${(poll
        .options[i].votes_count /
        poll.votes_count) *
        100}%"></div>
<label class="poll_result_label"><strong>${Math.round(
        (poll.options[i].votes_count / poll.votes_count) * 100
      ) || 0}%</strong> <span class="emoji_poss">${
        poll.options[i].title
      }</span></label>`;
    }
    if (poll.expired)
      poll_html += `<br/><span class="poll_footer">${poll.votes_count} ${__(
        "votes"
      )} &bull; ${__("Final results")}</span>`;
    else
      poll_html += `<br/><span class="poll_footer">${poll.votes_count} ${__(
        "votes"
      )} &bull; ${expires_string} ${__("left")}</span>`;
  } else {
    const poll_random = Math.round(Math.random() * 1000);
    poll_html = `<div class="poll_box poll_box_form poll_${
      poll.id
    }" data-poll="${poll.id}" data-random="${poll_random}" id="poll_${
      poll.id
    }_${poll_random}">`;
    for (var i = 0; i < poll.options.length; i++) {
      if (poll.multiple) {
        poll_html += `<input type="checkbox" id="poll_${
          poll.id
        }_${poll_random}_${i}" name="poll_${
          poll.id
        }" class="poll_vote_option checkbox">
<label for="poll_${
          poll.id
        }_${poll_random}_${i}" class="poll_vote_label poll_checkbox_label emoji_poss">${
          poll.options[i].title
        }</label><br/>`;
      } else {
        poll_html += `<div class="radiobox"><input type="radio" id="poll_${
          poll.id
        }_${poll_random}_${i}" name="poll_${poll.id}" class="poll_vote_option">
<label for="poll_${
          poll.id
        }_${poll_random}_${i}" class="poll_vote_label radiotext emoji_poss">${
          poll.options[i].title
        }</label></div>`;
      }
    }
    poll_html += `<button class="halcyon_button poll_vote"><span>${__(
      "Vote"
    )}</span></button>
  ...
  if (status.reblog === null) {
    for (i = 0; i < status.emojis.length; i++) {
      status.content = status.content.replace(
        new RegExp(":" + status.emojis[i].shortcode + ":", "g"),
        "<img src='" + status.emojis[i].url + "' class='emoji'>"
      );
    }
    status.account.display_name = htmlEscape(status.account.display_name);
    for (var i = 0; i < status.account.emojis.length; i++) {
      status.account.display_name = status.account.display_name.replace(
        new RegExp(":" + status.account.emojis[i].shortcode + ":", "g"),
        "<img src='" + status.account.emojis[i].url + "' class='emoji'>"
      );
    }
    for (var i = 0; i < status.mentions.length; i++) {
      if (status.mentions[i].acct.indexOf("@") == -1)
        status.content = status.content.replace(
          new RegExp('href="' + status.mentions[i].url + '"', "g"),
          'href="/@' +
            status.mentions[i].acct +
            "@" +
            current_instance +
            "?mid=" +
            status.mentions[i].id +
            '"'
        );
      else
        status.content = status.content.replace(
    ...
    if (status.account.acct.indexOf("@") == -1)
      status_account_link =
        "/@" +
        status.account.acct +
        "@home" +
      ...
      status_account_link =
        "/@" + status.account.acct + "?mid=" + status.account.id;
    const status_datetime = getRelativeDatetime(
        Date.now(),
        getConversionedDate(null, status.created_at)
      ),
    ...
    if (
      status.spoiler_text &&
      localStorage.setting_show_content_warning == "false"
    ) {
      (alart_text =
        "<span>" +
        status.spoiler_text +
        "</span><button class='cw_button'>" +
        __("SHOW MORE") +
        "</button>"),
        (article_option = "content_warning");
    } else if (
      status.spoiler_text &&
      localStorage.setting_show_content_warning == "true"
    ) {
      alart_text =
        "<span>" +
        status.spoiler_text +
    ...
    switch (status.visibility) {
      case "public":
        toot_privacy_mode = __("Public");
        toot_privacy_icon = "globe";
        break;
      case "unlisted":
        toot_privacy_mode = __("Unlisted");
        toot_privacy_icon = "unlock-alt";
        break;
      case "private":
        toot_privacy_mode = __("Followers-only");
        toot_privacy_icon = "lock";
        break;
      case "direct":
        toot_privacy_mode = __("Direct");
        toot_privacy_icon = "envelope";
        break;
    }
...
<span class="reaction_count boost_count">${toot_reblogs_count}</span>
    ...
    if (status.account.acct == current_acct) {
      var own_toot_buttons = `<li><a class="delete_button" tid="${
        status.id
      }">${__("Delete Toot")}</a></li>`;
      if (status.pinned == true) {
        own_toot_buttons += `<li><a class="unpin_button" tid="${
          status.id
        }">${__("Unpin Toot")}</a></li>`;
      } else {
        own_toot_buttons += `<li><a class="pin_button" tid="${status.id}">${__(
          "Pin Toot"
        )}</a></li>`;
      }
    } else {
      var own_toot_buttons = `<li><a class="mute_button" mid="${
        status.account.id
      }" sid="${status.id}">${__("Mute")} @${status.account.username}</a></li>
<li><a class="block_button" mid="${status.account.id}" sid="${status.id}">${__(
        "Block"
      )} @${status.account.username}</a></li>
<li><a class="addlist_button" mid="${status.account.id}" sid="${
        status.id
      }" display_name="${status.account.display_name}">${__("Add to list")} @${
        status.account.username
      }</a></li>
<li><a class="report_button" mid="${status.account.id}" sid="${
        status.id
      }" display_name="${status.account.display_name}">${__(
        "Report this Toot"
      )}</a></li>`;
    }
...
<ul>
<li><a class="copylink_button" url="${status.url}">${__(
      "Copy link to Toot"
    )}</a></li>
${own_toot_buttons}
</ul>
...
<button class="reply_button" tid="${status.id}" mentions='${JSON.stringify(
      status.mentions
    )}' display_name="${status.account.display_name}" privacy="${
      status.visibility
    }">
...
<i class="fa fa-fw fa-${toot_privacy_icon}" title="${toot_privacy_mode}"></i>
    ...
    for (i = 0; i < status.reblog.emojis.length; i++) {
      status.reblog.content = status.reblog.content.replace(
        new RegExp(":" + status.reblog.emojis[i].shortcode + ":", "g"),
        "<img src='" + status.reblog.emojis[i].url + "' class='emoji'>"
      );
    }
    status.account.display_name = htmlEscape(status.account.display_name);
    ...
    status.reblog.account.display_name = htmlEscape(
      status.reblog.account.display_name
    );
    for (i = 0; i < status.reblog.account.emojis.length; i++) {
      status.reblog.account.display_name = status.reblog.account.display_name.replace(
        new RegExp(":" + status.reblog.account.emojis[i].shortcode + ":", "g"),
        "<img src='" + status.reblog.account.emojis[i].url + "' class='emoji'>"
      );
    }
    for (var i = 0; i < status.reblog.mentions.length; i++) {
      if (status.reblog.mentions[i].acct.indexOf("@") == -1)
        status.reblog.content = status.reblog.content.replace(
          new RegExp('href="' + status.reblog.mentions[i].url + '"', "g"),
          'href="/@' +
            status.reblog.mentions[i].acct +
            "@" +
            current_instance +
            "?mid=" +
            status.reblog.mentions[i].id +
            '"'
        );
      else
        status.reblog.content = status.reblog.content.replace(
    ...
    const status_datetime = getRelativeDatetime(
        Date.now(),
        getConversionedDate(null, status.reblog.created_at)
      ),
      status_attr_datetime = getConversionedDate(
        null,
        status.reblog.created_at
      );
    var status_reblog_account_link, status_account_link;
    if (status.reblog.account.acct.indexOf("@") == -1)
      status_reblog_account_link =
        "/@" +
      ...
      status_reblog_account_link =
        "/@" + status.reblog.account.acct + "?mid=" + status.reblog.account.id;
    if (status.account.acct.indexOf("@") == -1)
      status_account_link =
        "/@" +
        status.account.acct +
    ...
    if (status.reblog.account.acct == current_acct) {
      ...
      if (status.reblog.pinned == true) {
        ...
        own_toot_buttons += `<li><a class="pin_button" tid="${
          status.reblog.id
        }">${__("Pin Toot")}</a></li>`;
      }
    } else {
      var own_toot_buttons = `<li><a class="mute_button" mid="${
        status.reblog.account.id
      }" sid="${status.reblog.id}">${__("Mute")} @${
        status.reblog.account.username
      }</a></li>
<li><a class="block_button" mid="${status.reblog.account.id}" sid="${
        status.reblog.id
      }">${__("Block")} @${status.reblog.account.username}</a></li>
<li><a class="addlist_button" mid="${status.reblog.account.id}" sid="${
        status.reblog.id
      }" display_name="${status.reblog.account.display_name}">${__(
        "Add to list"
      )} @${status.reblog.account.username}</a></li>
<li><a class="report_button" mid="${status.reblog.account.id}" sid="${
        status.reblog.id
      }" display_name="${status.reblog.account.display_name}">${__(
        "Report this Toot"
      )}</a></li>`;
    ...
    if (status.reblog.account.locked == true)
      account_state_icons += " <i class='fa fa-lock'></i>";
    if (status.reblog.account.bot == true)
      account_state_icons +=
        " <img src='/assets/images/robot.svg' class='emoji'>";
    const html = $(`
...
<a href="${status_account_link}">
<span class="emoji_poss"><i class="fa fa-fw fa-retweet"></i>${
      status.account.display_name
    } ${__("Boosted")}</span>
</a>
</div>
...
<div class="expand_menu invisible disallow_select">
<ul>
<li><a class="copylink_button" url="${status.reblog.url}" >${__(
      "Copy link to Toot"
    )}</a></li>
${own_toot_buttons}
</ul>
<ul>
<li><a href="${status.reblog.url}" target="_blank">${__(
      "View original"
    )}</a></li>
</ul>
</div>
...
<div class="toot_reaction">
<button class="reply_button" tid="${
      status.reblog.id
    }" mentions='${JSON.stringify(status.reblog.mentions)}' display_name="${
      status.reblog.account.display_name
    }" privacy="${status.reblog.visibility}">
<i class="fa fa-fw fa-reply"></i>
<span class="reaction_count reply_count">${toot_replies_count}</span>
</button>
</div>
<div class="toot_reaction">
<button class="boost_button" tid="${status.reblog.id}" reblogged="${
...
<div class="toot_reaction">
<button class="fav_button" tid="${status.reblog.id}" favourited="${
...
function notifications_template(NotificationObj) {
  ...
  if (NotificationObj.account.acct.indexOf("@") == -1)
    notice_author_link =
      "/@" +
      NotificationObj.account.acct +
      "@" +
      current_instance +
      "?mid=" +
      NotificationObj.account.id;
  else
    notice_author_link =
      "/@" +
      NotificationObj.account.acct +
  ...
  NotificationObj.account.display_name = htmlEscape(
    NotificationObj.account.display_name
  );
  for (i = 0; i < NotificationObj.account.emojis.length; i++) {
    NotificationObj.account.display_name = NotificationObj.account.display_name.replace(
      new RegExp(":" + NotificationObj.account.emojis[i].shortcode + ":", "g"),
      "<img src='" + NotificationObj.account.emojis[i].url + "' class='emoji'>"
    );
  }
  if (
    (NotificationObj.type === "favourite") |
    (NotificationObj.type === "reblog")
  ) {
    var toot_author_link;
    if (NotificationObj.status.account.acct.indexOf("@") == -1)
      toot_author_link =
        "/@" +
      ...
      toot_author_link =
        ...
        "?mid=" +
        NotificationObj.status.account.id;
    const toot_datetime = getRelativeDatetime(
        Date.now(),
        getConversionedDate(null, NotificationObj.status.created_at)
      ),
      toot_attr_datetime = getConversionedDate(
        null,
        NotificationObj.status.created_at
      );
    if (NotificationObj.type == "favourite") {
      for (i = 0; i < NotificationObj.status.emojis.length; i++) {
        NotificationObj.status.content = NotificationObj.status.content.replace(
          new RegExp(
            ":" + NotificationObj.status.emojis[i].shortcode + ":",
            "g"
          ),
          "<img src='" +
            NotificationObj.status.emojis[i].url +
            "' class='emoji'>"
        );
      }
      NotificationObj.status.account.display_name = htmlEscape(
        NotificationObj.status.account.display_name
      );
      for (i = 0; i < NotificationObj.status.account.emojis.length; i++) {
        NotificationObj.status.account.display_name = NotificationObj.status.account.display_name.replace(
          new RegExp(
            ":" + NotificationObj.status.account.emojis[i].shortcode + ":",
            "g"
          ),
          "<img src='" +
            NotificationObj.status.account.emojis[i].url +
            "' class='emoji'>"
        );
      }
      for (var i = 0; i < NotificationObj.status.mentions.length; i++) {
        if (NotificationObj.status.mentions[i].acct.indexOf("@") == -1)
          NotificationObj.status.content = NotificationObj.status.content.replace(
            new RegExp(
              'href="' + NotificationObj.status.mentions[i].url + '"',
              "g"
            ),
            'href="/@' +
              NotificationObj.status.mentions[i].acct +
              "@" +
              current_instance +
              "?mid=" +
              NotificationObj.status.mentions[i].id +
              '"'
          );
        else
          NotificationObj.status.content = NotificationObj.status.content.replace(
      ...
      if (NotificationObj.status.account.locked == true)
        account_state_icons += " <i class='fa fa-lock'></i>";
      if (NotificationObj.status.account.bot == true)
        account_state_icons +=
          " <img src='/assets/images/robot.svg' class='emoji'>";
      const html = `
<li sid="${
        NotificationObj.status.id
      }" class="notice_entry fav favourite toot_entry">
...
<span class="emoji_poss">${NotificationObj.account.display_name}</span> ${__(
...
<li sid="${
        NotificationObj.status.id
      }" class="notice_entry bos boost toot_entry">
<div class="notice_author_box">
<a href="${notice_author_link}">
<div class="icon_box">
<img src="${NotificationObj.account.avatar}">
...
<footer class="toot_footer"></footer>
    ...
    for (i = 0; i < NotificationObj.status.emojis.length; i++) {
      NotificationObj.status.content = NotificationObj.status.content.replace(
        new RegExp(":" + NotificationObj.status.emojis[i].shortcode + ":", "g"),
        "<img src='" + NotificationObj.status.emojis[i].url + "' class='emoji'>"
      );
    }
    NotificationObj.status.account.display_name = htmlEscape(
      NotificationObj.status.account.display_name
    ...
    if (
      NotificationObj.status.spoiler_text &&
      localStorage.setting_show_content_warning == "false"
    ) {
      (alart_text =
        "<span>" +
    ...
    if (NotificationObj.status.account.display_name.length == 0) {
      NotificationObj.status.account.display_name =
        NotificationObj.status.account.username;
    }
    ...
    if (toot_privacy_icon == "globe" || toot_privacy_icon == "unlock-alt") {
      toot_footer_width = " style='width:320px'";
      toot_reblog_button = `<div class="toot_reaction">
<button class="boost_button" tid="${NotificationObj.status.id}" reblogged="${
        NotificationObj.status.reblogged
      }">
<i class="fa fa-fw fa-retweet"></i>
<span class="reaction_count boost_count">${toot_reblogs_count}</span>
</button>
    ...
    if (NotificationObj.status.account.acct == current_acct) {
      ...
      var own_toot_buttons = `<li><a class="mute_button" mid="${
        NotificationObj.status.account.id
      }" sid="${NotificationObj.status.id}">${__("Mute")} @${
        NotificationObj.status.account.username
      }</a></li>
<li><a class="block_button" mid="${NotificationObj.status.account.id}" sid="${
        NotificationObj.status.id
      }">${__("Block")} @${NotificationObj.status.account.username}</a></li>
<li><a class="addlist_button" mid="${NotificationObj.status.account.id}" sid="${
        NotificationObj.status.id
      }" display_name="${NotificationObj.status.account.display_name}">${__(
        "Add to list"
      )} @${NotificationObj.status.account.username}</a></li>
<li><a class="report_button" mid="${NotificationObj.status.account.id}" sid="${
        NotificationObj.status.id
      }" display_name="${NotificationObj.account.display_name}">${__(
        "Report this Toot"
      )}</a></li>`;
    }
    var account_state_icons = "";
...
<div class="expand_menu invisible disallow_select">
<ul>
<li><a class="copylink_button" url="${NotificationObj.status.url}" >${__(
      "Copy link to Toot"
    )}</a></li>
${own_toot_buttons}
</ul>
<ul>
<li><a href="${NotificationObj.status.url}" target="_blank">${__(
      "View original"
    )}</a></li>
</ul>
</div>
...
<button class="reply_button" tid="${
      ...
      NotificationObj.status.mentions
    )}' display_name="${NotificationObj.account.display_name}" privacy="${
      NotificationObj.status.visibility
    }">
<i class="fa fa-fw fa-reply"></i>
<span class="reaction_count reply_count">${toot_replies_count}</span>
</button>
...
${toot_reblog_button}
<div class="toot_reaction">
<button class="fav_button" tid="${NotificationObj.status.id}" favourited="${
      NotificationObj.status.favourited
    }">
<i class="fa fa-fw fa-star"></i>
<span class="reaction_count fav_count">${toot_favourites_count}</span>
</button>
...
function follows_template(AccountObj) {
  ...
  if (AccountObj.acct.indexOf("@") == -1)
    profile_link =
      "/@" + AccountObj.acct + "@" + current_instance + "?mid=" + AccountObj.id;
...
<button class="halcyon_button follow_button action_button" mid="${
    AccountObj.id
  }">
...
<form id="reply_status_form" name="reply_status_form" class="status_form" sid="${
      status.id
...
<input class="status_spoiler invisible" name="status_spoiler" placeholder="${__(
      "Content warning"
    )}" data-random="${Math.round(Math.random() * 1000)}" type="text"/>
</div>
<div class="status_main">
<!-- text area -->
<div class="status_textarea">
<textarea class="emoji_poss" name="status_textarea" placeholder="${__(
      "Toot your reply"
    )}" data-random="${Math.round(Math.random() * 1000)}"></textarea>
...
<i class="fa fa-circle-o"></i> <input name="options[]" type="text" class="disallow_enter textfield poll_field" maxlength="25"><br/>
<i class="fa fa-circle-o"></i> <input name="options[]" type="text" class="disallow_enter textfield poll_field" maxlength="25"><br/>
<div style="height:32px;display:inline-block;padding-top:10px">${__(
      "Expires in"
    )} </div>
<div style="float:right;margin-right:5px"><div class="poll_time"><input type="number" min="0" max="7" placeholder="0-7" class="poll_days">${__(
      "Days"
    )}</div>
<div class="poll_time"><input type="number" min="0" max="24" placeholder="0-24" class="poll_hours">${__(
      "Hours"
    )}</div>
<div class="poll_time"><input type="number" min="0" max="60" placeholder="0-60" class="poll_mins">${__(
      "Minutes"
    )}</div></div><br/>
...
<input id="reply_status_form_submit" class="submit_status" type="button" class="invisible"/>
    ...
    history.pushState(
      null,
      null,
      status_account_link.replace("?mid=", "/status/" + status.id + "?mid=")
    );
    html.find(".toot_article").append(media_views);
    html.find(".toot_article").append(poll_object);
    return html;
...
<form id="reply_status_form" name="reply_status_form" class="status_form" sid="${
      status.reblog.id
    }" mentions='${JSON.stringify(status.reblog.mentions)}'>
<div class="status_left icon_box">
<img class="js_current_profile_image" src="${current_avatar}">
</div>
<div class="status_top">
...
`);
    history.pushState(
      null,
      null,
      status_reblog_account_link.replace(
        "?mid=",
        "/status/" + status.reblog.id + "?mid="
      )
    );
    html.find(".toot_article").append(media_views);
    html.find(".toot_article").append(poll_object);
...
<img src="${media}">
    ...
    if (media == status.media_attachments.length - 1)
      hideforward = " style='display:none'";
    const status_template = timeline_template(status).html(),
      html = `<div class="media_detail" pictures='${JSON.stringify(
        pictures
      )}' cid="${media}">
<div class="media_box">
